...
char *getJsonError(cJSON *json)
{
    ...
    if (cJSON_HasObjectItem(json, "error")) {
        ...
        cJSON *error_type = cJSON_GetObjectItem(error, "type");
        cJSON *error_message = cJSON_GetObjectItem(error, "message");
        char *errorMsg = NULL;
        asprintf(&errorMsg, "code=%s type=%s message=%s",
                 cJSON_IsString(error_code) ? cJSON_GetStringValue(error_code) : "unknown",
                 cJSON_IsString(error_type) ? cJSON_GetStringValue(error_type) : "unknown",
                 cJSON_IsString(error_message) ? cJSON_GetStringValue(error_message) : "unknown");
        OPENAI_ERROR_CHECK(errorMsg != NULL, "asprintf failed!", NULL);
        return errorMsg;
    }
    return NULL;
...
static cJSON *createMultiModalChatMessage(const char *role, const char *type, const char *value)
{
    ...
    /* DeepSeek's OpenAI-compatible chat API expects plain text turns as
     * a string.  The content-part array is reserved for multimodal inputs. */
    if (strcmp(type, "text") == 0) {
        if (cJSON_AddStringToObject(message, "content", value) == NULL) {
            cJSON_Delete(message);
            ESP_LOGE(TAG, "Failed to add text content!");
            return NULL;
        }
        return message;
    }
...
static OpenAI_StringResponse_t *OpenAI_ChatCompletionMultiModalMessage(OpenAI_ChatCompletion_t *chatCompletion, const char *type, const char *contentValue, bool save)
{
    ...
    if (save) {
        // Persist both sides of a successful turn.  Saving only the assistant
        // response makes the next request start with an assistant role, which
        // OpenAI-compatible APIs reject as an invalid conversation history.
        OpenAI_StringResponse_t *r = OpenAI_StringResponseCreate(res);
        if (r && r->getLen(r)) {
            cJSON *user_msg = createMultiModalChatMessage(role, type, contentValue);
            if (user_msg) {
                if (!cJSON_AddItemToArray(_chatCompletion->messages, user_msg)) {
                    cJSON_Delete(user_msg);
                    ESP_LOGE(TAG, "Failed to save user message!");
                }
            } else {
            ...
            if (assistant_msg) {
                if (!cJSON_AddItemToArray(_chatCompletion->messages, assistant_msg)) {
                    cJSON_Delete(assistant_msg);
                    ESP_LOGE(TAG, "Failed to save assistant message!");
                }
            } else {
