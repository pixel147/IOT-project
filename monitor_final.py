"""Monitor for 120 seconds - focus on OTA, MCP, wake word."""
...
OUTPUT_FILE = r'E:\esp_practice\first_practice\monitor_final.txt'
log_lines = []
        ...
        try:
            if ser.in_waiting:
                ...
                while b'\n' in buf:
                    ...
                    u = decoded.upper()
                    if any(kw in u for kw in ['XIAOZHI','WS_','MCP','OTA','SESSION','STT','TTS',
                                               'WAKE','STATE=','REGISTER','DISCONN','ERROR','PANIC']):
                        print(decoded)
        except:
            break
