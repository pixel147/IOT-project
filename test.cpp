...
static void voice_task(void *arg)
{
    ...
    /* Wait for wake word */
    ui_chat_voice_set_status("Voice: say ä½ å¥½å°æº");
    ...
    while (s_voice.websocket && esp_websocket_client_is_connected(s_voice.websocket)) {
        ...
        /* Get required feed chunk size from AFE */
        int feed_chunk = s_voice.afe_iface->get_feed_chunksize(s_voice.afe);
        /* Allocate buffer on heap to avoid stack overflow */
        int16_t *afe_buf = (int16_t *)heap_caps_malloc(feed_chunk * sizeof(int16_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        if (!afe_buf) {
            vTaskDelay(pdMS_TO_TICKS(10));
            continue;
        }
        /* Read PCM audio from codec (blocking, with timeout) */
        int ret = esp_codec_dev_read(s_voice.codec, afe_buf, feed_chunk * sizeof(int16_t));
        if (ret == feed_chunk * (int)sizeof(int16_t)) {
        ...
        if (result->wakenet_state > 0 && !s_voice.listening) {
            ESP_LOGI(TAG, "Wake word triggered! state=%d", result->wakenet_state);
            char listen[192];
            snprintf(listen, sizeof(listen),
...
static void voice_hardware_init_task(void *arg)
{
    ...
    ui_chat_voice_set_status("Voice: init I2S");
    {
        ...
        i2s_cfg.slot_cfg = I2S_STD_MSB_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_STEREO);
        i2s_cfg.gpio_cfg = {
    ...
    ui_chat_voice_set_status("Voice: say 浣犲ソ灏忔櫤");
